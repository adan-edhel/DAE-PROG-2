...
struct Matrix2x3 final
{
	...
	explicit Matrix2x3( Vector2 dirX = Vector2{ 1, 0 }, Vector2 dirY = Vector2{ 0, 1 }, Vector2 origTrans = Vector2{ 0, 0 } );
	...
	// Vector2 vTransformed = mat.Transform(v);
	...
	// Converts this matrix into a Translation matrix
	void SetAsTranslate( Vector2 pt );
	...
	// Instantiate a scale matrix: 
	// Matrix matScale = Matrix::Scaling( Vector2(2.0f,-3.0f) );
	static Matrix2x3 CreateScalingMatrix( Vector2 scaleVector );
	// Instantiate a translation matrix: 
	// Matrix matTrans = Matrix::Translation( Vector2(2.0f,3.0f) );
	static Matrix2x3 CreateTranslationMatrix( Vector2 origin );
	// Instantiate a translation matrix: 
	...
	// Datamembers
	// -------------------------
	Vector2 dirX;	// The first matrix vector (the "x-axis"), 1st column
	Vector2 dirY;	// The second matrix vector (the "y-axis"), second column
	Vector2 orig; 	// The origin of  the coordinate matrix (the "translation"), third column
};
