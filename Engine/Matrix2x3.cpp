...
Matrix2x3::Matrix2x3(Vector2 dirX, Vector2 dirY, Vector2 orig)
	: dirX{ dirX }, dirY{ dirY }, orig{ orig }
{}

Matrix2x3::Matrix2x3(float e1X, float e1Y, float e2X, float e2Y, float oX, float oY) 
	: Matrix2x3{ Vector2{e1X, e1Y}, Vector2{e2X, e2Y}, Vector2{oX, oY} }
{}

Vector2 Matrix2x3::Transform(const Vector2& vector) const
{
	return Vector2{ vector.x * dirX + vector.y * dirY };
}

Point2f Matrix2x3::Transform(const Point2f& point) const
{
	Vector2 v{ Transform( Vector2{ point } ) + orig };
	return  v.ToPoint2f();
}
...
Matrix2x3 Matrix2x3::Inverse() const
{
	...
	return Matrix2x3{
		Vector2(+dirY.y, -dirX.y) / det,
		Vector2(-dirY.x, +dirX.x) / det,
		Vector2(dirY.x * orig.y - dirY.y * orig.x, -(dirX.x * orig.y - dirX.y * orig.x)) / det
	};
}
...
void Matrix2x3::SetAsRotate(float degrees)
{
	...
	dirX = Vector2{ cos( radians ), sin( radians ) };
	dirY = Vector2{ -sin( radians ), cos( radians ) };
	orig = Vector2{ 0, 0 };
}
...
void Matrix2x3::SetAsTranslate(Vector2 pt)
{
	dirX = Vector2{ 1, 0 };
	dirY = Vector2{ 0, 1 };
	orig = Vector2{ pt.x, pt.y };
}

void Matrix2x3::SetAsScale(float scaleX, float scaleY)
{
	dirX = Vector2{ scaleX, 0 };
	dirY = Vector2{ 0, scaleY };
	orig = Vector2{ 0, 0 };
}
...
{
	float radians = degrees * 3.1415926535f / 180;
	return Matrix2x3( Vector2{ cos( radians ), sin( radians ) }, Vector2{ -sin(radians), cos( radians ) }, Vector2{} );
}

Matrix2x3 Matrix2x3::CreateIdentityMatrix()
{
	return Matrix2x3( Vector2{ 1, 0 }, Vector2{ 0, 1 }, Vector2{} );
}
...
Matrix2x3 Matrix2x3::CreateScalingMatrix(Vector2 scaleVector)
{
...
{
	return Matrix2x3( Vector2{ scaleX, 0 }, Vector2{ 0, scaleY }, Vector2{} );
}

Matrix2x3 Matrix2x3::CreateTranslationMatrix(Vector2 origin)
{
	return Matrix2x3( Vector2{ 1, 0 }, Vector2{ 0, 1 }, origin );
}

Matrix2x3 Matrix2x3::CreateTranslationMatrix(float tx, float ty)
{
	return CreateTranslationMatrix( Vector2{ tx, ty } );
}
...
{
	return Matrix2x3{
		Vector2{ rhs.dirX.x * lhs.dirX.x + rhs.dirX.y * lhs.dirY.x, rhs.dirX.x * lhs.dirX.y + rhs.dirX.y * lhs.dirY.y },
		Vector2{ rhs.dirY.x * lhs.dirX.x + rhs.dirY.y * lhs.dirY.x, rhs.dirY.x * lhs.dirX.y + rhs.dirY.y * lhs.dirY.y },
		Vector2{ rhs.orig.x * lhs.dirX.x + rhs.orig.y * lhs.dirY.x + lhs.orig.x, rhs.orig.x * lhs.dirX.y + rhs.orig.y * lhs.dirY.y + lhs.orig.y }
	};
}
